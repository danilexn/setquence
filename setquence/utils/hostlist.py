...
def remove_duplicates(hostlist):
    ...
    for ((prefix, suffix), group) in itertools.groupby(sortlist, key=lambda x: x[0]):
        ...
        if suffix is None:
            ...
            if len(range_list) == 1 and range_list[0][0] == range_list[0][1]:
                ...
                results.append((prefix, "%0*d%s" % (range_list[0][2], range_list[0][0], suffix)))
            else:
                # General case where high > low
                results.append(
                    (
                        prefix,
                        "[" + ",".join([format_range(hostlist, h, w) for hostlist, h, w in range_list]) + "]" + suffix,
                    )
                )
...
def numerically_sorted(hostlist):
    """Sort a list of hosts numerically.
    E.g. sorted order should be n1, n2, n10; not n1, n10, n2.
    """

    return sorted(hostlist, key=numeric_sort_key)
